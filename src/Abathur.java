...
//		System.out.println("Abathur.sortMap made list " + entryList.toString());
	...
	public static String[] topTenKeywords(HashMap<String, Integer> map) {
		ArrayList<String[]> list = convertMapToSortedArrayList(map);
		String[] topKeywords = new String[10];
		StringBuilder sb = new StringBuilder();
		for (int i = 0; i < 10; i++) {
			try {
				topKeywords[i] = list.get(i)[0];
			} catch (NullPointerException n) {
				//might get an NPE if there aren't 5 keywords
			}
		}
		System.out.println("Abathur.topTenKw -- generated from keyword HashMap: " + sb.toString());
		return topKeywords;
	}
