...
public class RegexUtils {
	public static HashSet<String> findEmails(String input, String origin) {
		...
		for (String word : splitWordArray) {
			...
			if (word != null) {
				// if the word fits the format of an email
				// (e.g.john.doe@email.com), consider it for HashSet
				...
				// regex just negates chars that aren't valid for usernames
	...
	public static HashSet<PersonObject> findNames(String input) {
		...
		Pattern pattern = Pattern.compile("([A-Z][a-z\\-]+)_([A-Z]\\.?_)?([A-Z][A-Za-z\\-\\']+)");
		...
		while (matcher.find()) {
			try {
				...
				boolean flag = true;
				// flag to check if the potential PersonObject is valid - has
				// first & last name)
				...
				if (matcher.group(1) != null && !Utils.wordInFilter(matcher.group(1), Filters.frequentEnglish)
					...
					// System.out.println("REJECT -- First Name " +
					// matcher.group(1));
					flag = false;
				}
				...
				if (matcher.group(3) != null && !Utils.wordInFilter(matcher.group(3), Filters.frequentEnglish)
					...
					// System.out.println("REJECT -- Last name" +
					// matcher.group(2));
					flag = false;
				}
...
//					System.out.println("Creating new Person Object: " + fName + ", " + lName + ", \n");
	...
	public static boolean unwantedUrlDestination(String url) {
		if (url.matches(".+(.jpg|.jpeg|.png|.gif|.pdf|.stm|.aspx|#|.xml|.json){1}$")
			...
			// System.out.println("RegexUtils.unwanted deemed <" + url + "> to
			// be undesireable destination url");
			return true;
		}
		return false;
	}
	...
	public static String cleanText(String input, String origin, boolean filterMonth, boolean filterDay,
		...
		for (String word : dirtyWordArray) {
			if (word.matches("[a-z[A-Z]]+") && !word.matches("null") && !origin.contains(word.toLowerCase())
				...
				// if the word is made up entirely of alphabet chars, is not a
				// date, does not match name of the domain
		...
		// System.out.println("RegexUtils.cleanText -- cleanString is: " +
		// cleanString);
	...
	private static boolean passedKeywordFilter(String word, boolean filterMonth, boolean filterDay,
		...
		if (Utils.wordInBinarySearchFilter(word, Filters.frequentEnglish)) {
			return false;
		}

		if (Utils.wordInBinarySearchFilter(word, Filters.internetCommon)) {
			return false;
		}
